...
function makeMechanics(worlds, shootDelay, levelId) {
  ...
  if (worlds.includes('forest')) {
    ...
    m.trapCount = worlds.length > 1 ? 4 : 8; // max 8 σε pure forest, 4 σε mix
    m.trapInterval = Math.round(3000 - t * 1500); // 3s → 1.5s
  }
  ...
  if (worlds.includes('poison')) {
    ...
    m.poisonInterval = Math.round(3000 - t * 1500);
...
for (let i = 0; i < 10; i++) {
  ...
  const shootDelay = Math.round(1150 - i * 33);
